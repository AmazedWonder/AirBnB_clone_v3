#!/usr/bin/python3
""" Create a flask view for Place objects that
...
def place_views(city_id):
    """ Retrieves all Place objects and returns a JSON response """
...
def places_id_views(place_id):
    """ Retrieves a specific Place object by its
...
def delete_place(place_id):
    """ Deletes a specific Place object by its amenity_id
...
def create_place(city_id):
    """ Creates a new Place object based on the JSON data provided """
    get_id = storage.get(City, city_id)
    if get_id is None:
        abort(404)
...
def update_place(place_id):
    """ Updates a specific Place object by its amenity_id """
    get_id = storage.get(Place, place_id)
    if get_id is None:
        abort(404)
    data_req = request.get_json()
